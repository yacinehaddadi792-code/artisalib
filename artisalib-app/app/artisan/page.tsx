...
export default async function ArtisanDashboard() {
  ...
  const [bookings, reviews, quotes, requests] = await Promise.all([
    ...
    <main className="mx-auto max-w-7xl px-6 py-10 lg:px-8">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="mt-2 text-zinc-600">
            Plan {user.artisanProfile.subscriptionPlan ?? 'BASIC'} · statut{' '}
            {user.artisanProfile.subscriptionStatus ?? 'INCOMPLETE'}
          </p>
        </div>
        ...
        <form action="/api/stripe/create-checkout-session" method="POST">
          <input
            type="hidden"
            name="plan"
            value={user.artisanProfile.subscriptionPlan ?? 'BASIC'}
          />
          <button className="btn-primary">
            Activer / gérer mon abonnement Stripe
          </button>
        </form>
      </div>
      ...
      <div className="mt-8 grid gap-4 md:grid-cols-5">
        <div className="card p-6">
          <p className="text-xs uppercase tracking-[0.15em] text-zinc-500">Demandes clients</p>
          <p className="mt-2 text-4xl font-black [font-family:var(--font-playfair)]">{requests.length}</p>
        </div>

        <div className="card p-6">
          <p className="text-xs uppercase tracking-[0.15em] text-zinc-500">Rendez-vous</p>
          <p className="mt-2 text-4xl font-black [font-family:var(--font-playfair)]">{bookings.length}</p>
        </div>

        <div className="card p-6">
          <p className="text-xs uppercase tracking-[0.15em] text-zinc-500">Devis</p>
          <p className="mt-2 text-4xl font-black [font-family:var(--font-playfair)]">{quotes.length}</p>
        </div>
      ...
      <section className="card mt-8 overflow-hidden">
        <div className="border-b border-zinc-200 px-6 py-4">
          <h2 className="text-xl font-bold">Demandes clients disponibles</h2>
          <p className="mt-1 text-sm text-zinc-600">
            Voici les demandes envoyées par les clients. Contactez-les pour proposer un devis.
          </p>
        </div>

        <div className="divide-y divide-zinc-200">
          {requests.length === 0 ? (
            <p className="px-6 py-6 text-zinc-500">Aucune demande pour le moment.</p>
          ) : (
              ...
              <div key={request.id} className="px-6 py-5">
                <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                  <div>
                    ...
                    <p className="mt-2 text-zinc-700">{request.description}</p>
                    ...
                    <div className="mt-4 text-sm text-zinc-500">
                      ...
                      {request.user.phone && <p>Téléphone : {request.user.phone}</p>}
                    </div>
                  </div>

                  <a
                    href={`mailto:${request.user.email}?subject=Votre demande sur Artisalib`}
            ...
            {bookings.length === 0 ? (
              ...
              bookings.map((booking) => (
                ...
                >
                  ...
                  <span className="rounded-full bg-zinc-100 px-3 py-2 text-xs font-semibold uppercase tracking-wide text-zinc-700">
                    {booking.status}
                  </span>
                </div>
              ))
